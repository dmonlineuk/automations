...
1. Update a copy of .env.example (named as .env) - this needs to have the actual domain name and hostame, as well as paths to secret and key files
1. Copy and edit the service and timer files into /etc/systemd/system/
1. Enable timer and review logs
```bash
# Enable
...
```
1. Check the logs from the script's internal code using `journalctl -t dynamicdns`
